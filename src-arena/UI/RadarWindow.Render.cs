    ...
    internal static partial class RadarWindow
    {
        ...
        private static void DrawMainMenuBar()
        {
            ...
            if (ImGui.BeginMenu("View"))
            {
                ...
                // Monitor picker — pin the ESP overlay to whichever screen the
                // user wants (e.g. game running on monitor 1, radar on 2, ESP
                // fuser on 3). Hidden when only one monitor is detected.
                var monitors = EspWindow.ListMonitors();
                if (monitors.Count > 1 && ImGui.BeginMenu("ESP Monitor"))
                {
                    int current = Math.Clamp(Config.EspMonitorIndex, 0, monitors.Count - 1);
                    for (int i = 0; i < monitors.Count; i++)
                    {
                        var m = monitors[i];
                        // Silk's IMonitor.Name can be null on some drivers; fall back to an index label.
                        string mname = string.IsNullOrEmpty(m.Name) ? $"Monitor {i + 1}" : m.Name!;
                        string label = $"#{i}: {mname}  ({m.Bounds.Size.X}x{m.Bounds.Size.Y} @ {m.Bounds.Origin.X},{m.Bounds.Origin.Y})";
                        if (ImGui.MenuItem(label, null, i == current))
                        {
                            Config.EspMonitorIndex = i;
                            if (EspWindow.IsOpen)
                            {
                                EspWindow.Close();
                                EspWindow.Open();
            ...
            // Right-aligned: MatchMode | MapName | FPS
            var gw = Memory.CurrentGameWorld;
            string mapName = gw?.MapName ?? "No Map";
            string mode    = gw is null ? "—" : gw.MatchMode.ToString();
            string right = $"{mode}  |  {mapName}  |  {_fps} FPS";
