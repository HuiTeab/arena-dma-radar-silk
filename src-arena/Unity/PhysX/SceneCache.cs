...
using eft_dma_radar.Arena.Misc;
...
namespace eft_dma_radar.Arena.Unity.PhysX
{
    ...
    internal static class SceneCache
    {
            ...
            _ = Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        // Persist the fresh snapshot for the next attach. The
                        // save runs synchronously on this task — it's typically
        ...
        private static SceneSnapshot? BuildOnce(string mapId)
        {
            ...
            if (!Memory.TryReadPtr(unityBase + PhysXOffsets.PhysXSdkRva, out var npPhysics, false)
                || !npPhysics.IsValidVirtualAddress())
            {
                _lastError = $"NpPhysics pointer invalid (rva=0x{PhysXOffsets.PhysXSdkRva:X8})";
                return null;
            }
            _lastNpPhysics = npPhysics;
            _lastSdkRva = PhysXOffsets.PhysXSdkRva;
            ...
            ulong[]? actorPtrs;
            try { actorPtrs = Memory.ReadArray<ulong>(actorsArrayPtr, (int)gameplayActorCount, false); }
            catch (Exception ex)
            {
            ...
            foreach (var actorPtr in actorPtrs)
            {
                if (!actorPtr.IsValidVirtualAddress()) { skippedReadError++; continue; }

                if (sampleActor == 0) sampleActor = actorPtr;

                if (!Memory.TryReadValue<ushort>(actorPtr + PhysXOffsets.PxBase_ConcreteType, out var typeRaw, false))
                {
                    skippedReadError++;
                    continue;
                }
                ...
                // Quick pre-check: skip actors whose NpShapeManager reports zero shapes.
                // This avoids promoting them to the step counters and keeps logs legible.
                if (!Memory.TryReadValue<ushort>(
                        actorPtr + PhysXOffsets.PxRigidActor_ShapeManager + PhysXOffsets.NpShapeManager_ShapesCount,
                        out var shapeCount, false) || shapeCount == 0)
                {
                    skippedZeroShapes++;
                    continue;
                }

                // Diagnostic: how many actors carry > 1 shape. If this counter
                // is large, the current first-shape-only ingest might be
                // dropping real geometry from secondary slots and a full
                // multi-shape walk is worth the refactor.
                if (shapeCount > 1) multiShapeActors++;

                if (firstShapedActor == 0) firstShapedActor = actorPtr;

                // Sample shape pointers at stride for the multi-shape diagnostic.
                if (sampledShapePtrs.Count < MaxShapeSamples
                    && (shapedActorsSeen % ShapeSampleStride) == 0)
                {
                    if (Memory.TryReadPtr(
                            actorPtr + PhysXOffsets.PxRigidActor_ShapeManager + PhysXOffsets.NpShapeManager_ShapesSingle,
                            out var samplePtr, false)
                        && samplePtr.IsValidVirtualAddress())
                    {
                        sampledShapePtrs.Add(samplePtr);
                    }
                }
                shapedActorsSeen++;

                var result = TryBuildActor(mapId, actorPtr, concrete,
                    meshes, convexes, heightFields,
                    meshDedup, convexDedup, hfDedup, out var built);
                stepCounts[(int)result]++;
                if (result == BuildActorResult.Ok)
                {
                    actors.Add(built);
                    processed++;
                }
