...
namespace eft_dma_radar.Arena.UI
{
    ...
    internal static class EspWindow
    {
        ...
        /// <summary>
        /// Returns every connected monitor in <see cref="Silk.NET.Windowing.Monitor.GetMonitors"/>
        /// order (primary at index 0). Empty list when the platform refuses to
        /// enumerate — callers should treat that as "primary only" and skip the
        /// picker UI.
        /// </summary>
        public static IReadOnlyList<IMonitor> ListMonitors() => SafeGetMonitors();

        private static IReadOnlyList<IMonitor> SafeGetMonitors()
        {
            try
            {
                var list = new List<IMonitor>();
                foreach (var m in Silk.NET.Windowing.Monitor.GetMonitors(null))
                    list.Add(m);
                return list;
            }
            catch (Exception ex)
            {
                Log.WriteRateLimited(AppLogLevel.Warning, "esp_monitor_enum", TimeSpan.FromMinutes(1),
                    $"[EspWindow] Monitor enumeration failed: {ex.Message}");
        ...
        private static void RunWindow()
        {
            try
            {
                // Pick target monitor — clamped to available range so an unplugged
                // monitor falls back to the primary one (index 0) cleanly.
                var monitors = SafeGetMonitors();
                int monIdx = Math.Clamp(Config.EspMonitorIndex, 0, Math.Max(0, monitors.Count - 1));
                IMonitor? mon = monitors.Count > 0 ? monitors[monIdx] : null;
                var monBounds = mon?.Bounds
                    ?? new Rectangle<int>(0, 0, Config.GameMonitorWidth, Config.GameMonitorHeight);
                int monW = monBounds.Size.X > 0 ? monBounds.Size.X : Config.GameMonitorWidth;
                int monH = monBounds.Size.Y > 0 ? monBounds.Size.Y : Config.GameMonitorHeight;

                var options = WindowOptions.Default;
                bool fullscreen = Config.EspFullscreen;
                if (fullscreen)
                {
                    // Borderless "fullscreen-window" covering the target monitor's full
                    // bounds. Using WindowState.Fullscreen would steal exclusive mode
                    // and stop the user from clicking the radar window underneath; the
                    // ESP is conceptually an overlay so borderless is the right choice.
                    options.Size = new Vector2D<int>(monW, monH);
                    options.Position = new Vector2D<int>(monBounds.Origin.X, monBounds.Origin.Y);
                ...
                else
                {
                    ...
                    // Center the windowed ESP on the target monitor.
                    options.Size = new Vector2D<int>(w, h);
                    options.Position = new Vector2D<int>(
                        monBounds.Origin.X + Math.Max(0, (monW - w) / 2),
                        monBounds.Origin.Y + Math.Max(0, (monH - h) / 2));
