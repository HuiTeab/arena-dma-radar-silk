                ...
                case Key.F7:
                    {
                        ...
                        if (gw is null)
                        {
                            Log.WriteLine("[ArenaProgram] F7: no active match — match dump skipped.");
                        }
                        else if (!Config.EnableMatchDump)
                        {
                            Log.WriteLine("[ArenaProgram] F7: EnableMatchDump is off — set it true in config.json to enable.");
                        }
                        else
                        {
                            Log.WriteLine("[ArenaProgram] F7: triggering match dump...");
                            gw.DumpMatchNow();
                        }
                        break;
                    }
